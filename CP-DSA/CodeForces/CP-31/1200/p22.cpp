...
void solve()
{
    ...
    // cout << blanks - min(blanks, k) + 1 << endl;

    // Now, how many ops do we require to fix each problematic gap
    
    vector<int> ops_req;
    rep(i,n-1){
        int gap = a[i+1] - a[i];

        // If diff is <= x then they belong to same group
        if(gap > x){
            int ops = (gap +x - 1)/x - 1; //ceil(gap/x) - 1
            // one segment is not needed 
            ops_req.push_back(ops);
        }
    }

    sort(all(ops_req));// we'll fix the cheapest ones therefore sorting
    int groups = ops_req.size()+1; // as each problematic gap gives one extra group

    // if we have enough to fix cheapest completely we merge the two groups seperated by it

    for(auto &it : ops_req){
        if(k >= it){
            k -= it; // spend required operation on this gap
            groups--;
        }else{
            break;
        }
