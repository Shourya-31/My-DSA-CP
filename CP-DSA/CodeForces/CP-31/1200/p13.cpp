...
void solve()
{
    ...
    while (t--)
    {
        ...
        // vector<bool> is_infected(m+1,false);

        // for(int i=0; i<n; i++){
        //     if(q[i] == a[i]){
        //         is_infected.push_back(a[i]);
        //         is_infected[i] = true;
        //     }
        // }

        // Treat the circle as gaps between infected
        // houses, sort the gaps by size, and greedily 
        // save the largest gaps first because every day
        // all gaps shrink by 2 due to infection spreading 
        // from both ends.
