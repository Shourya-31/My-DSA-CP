...
void solve()
{
    ...
    //Each index stores two things: its current value (first)
    // and the query number when it was last individually updated (second).
    ...
    // Keeps track of the most recent Type-2 operation. 
    // first stores the assigned value, while second stores
    // the query number when that assignment happened.
    long long sum = 0;
    for (int i = 0; i < n; i++)
        sum += v[i].first;

    for (int qry = 1; qry <= k; ++qry)
    // The variable qry also acts as a timestamp for tracking updates.
    {
        int type;
        cin >> type;
        ...
        if(type == 1){
            ...
            // Checks whether this index was updated after
            // the last global assignment. If yes, its stored value is current; 
            // otherwise, its effective value is the last globally assigned value.

            v[ind].first = val;
            v[ind].second = qry;
            // Stores the new value and records that this index was individually updated during the current query.
        }
        else{
            int val;
            cin>>val;
            globalVal.first = val;
            globalVal.second = qry;
            // Instead of modifying every element, remembers the global value and the query number when it was assigned.
    ...
    // while (k--)
    // {
    //     // Replace ith element with int x in type-1
    //     // Replace each element with int x in type-2
    //     int type;
    //     cin >> type;
