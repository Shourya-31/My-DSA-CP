...
void solve()
{
    ...
    while (t--)
    {
        ...
        // he removes either the first or the last element of the array.
        // total sum of the array is equal to s after performing all the operations
        // Find Min. Ops

        // in this case the sum should be equal to k after the min operations are completed
        int sum = 0;

        // so we shall use the 2 pointers approach and also the binary search approach to find the min operations
        // we must fine sub array with sum equal to k
        // now the catch comes in the fact that we can remove either first or last elemnt of the array
        
        // using 2 pointers to find sub array with sum equal to k

        // Now its not about which elements i should remove,
        // its about which continuous sub array i should keep so sum = k
        // lets try sliding window 
        
        int total_sum = accumulate(all(a), 0LL);
        if(total_sum < k){
            pm;
            continue;
        }
        ...
        // If the longest valid subarray has length longest, 
        // then all the remaining n - longest elements must be
        // removed from the left and/or right.
        // Therefore, n - longest gives the minimum number of operations required.
        ...
        // // but what if we find prefix sum
        // // we can find the prefix sum and then we can find the suffix sum and then we can find the min operations

        // // with a map to track the prefix sum and its index
        // map<int, int> prefix_sum;
        // prefix_sum[0] = 0; // to handle the case when the prefix sum is equal to k at the start

        // rep(i,n){
        //     sum += a[i];
        //     prefix_sum[sum] = i+1; // store the index + 1 to handle the case when the prefix sum is equal to k at the start
        // }
    }
}
