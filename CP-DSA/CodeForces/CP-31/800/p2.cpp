...
// Approach:
// Since we need to get the minimum volume of fuel tank.
// Therefore we can measure distance like: the max distance between the 2 gas stations.
// And the distance we need to go to the first gas station
// Also the thing is, we need to reach the final point and return from there to the nearesrt gas station. Uska bhi dhyaan rakhna padega
// So lets build the solution now:
...
int main(){
   ...
   while(t--){
    ...
    // Distance from start (0) to first station
    ans = max(ans,a[0]);

    // Max distance between consecutive stations
    for(int i = 1; i < n; i++){
        ans = max(ans,a[i]-a[i-1]);
    }

    // Distance from last stations to x and back
