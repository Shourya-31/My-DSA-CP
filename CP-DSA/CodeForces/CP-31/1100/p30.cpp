...
void solve()
{
    ...
    while (t--)
    {
        ...
        auto isPalindrome = [](vector<int> a)
        {
            int left = 0;
            int right = a.size() - 1;
        ...
        for (int i = 0; i < n; i++)
        {
            if (a[i] != a[n - i - 1])
            {
                // a.pop_back();
                del.push_back(a[i]);
                del.push_back(a[n - i - 1]);
                break;
            }
        }

        if (!isPalindrome(a) && !del.empty())
        {
            a.erase(remove(a.begin(), a.end(), del[0]), a.end());
        }

        if (!isPalindrome(b) && !del.empty())
        {
            b.erase(remove(b.begin(), b.end(), del[1]), b.end());
        }
