...
void solve()
{
    ...
    while (t--)
    {
        ...
        // Goal: the minimum number of operations
        // needed to make equal all the elements of
        // the array with the given operation.

        // Something from right is being assigned to something to left
        // the equal element will be last element of the array.
        // we will select a sub-array where in right most part
        // it contains 3 and the other elements may or may not.

        // we will select maximum sub-array for the sake of minimum operations
        ...
        // If we reached beyond the left side, every element was equal.

        while (i >= 0)
        {
            i -= (n - i - 1);
            // Instead of actually modifying the array, we simply
            // move i left by len positions to represent that those
            // positions are now considered "covered" by the good suffix.
            ans++;
            while (i >= 0 && a[i] == a[n - 1])
            {
                i--;
